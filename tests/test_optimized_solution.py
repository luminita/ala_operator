"""
Implements unit tests for the module optimized_solution
"""
import os
import sys
import unittest
...
class TestLoadData(unittest.TestCase):
    """ Test that the function load_data in OptimizedSolution loads correctly 
    an input file"""
    def setUp(self):
        unittest.TestCase.setUp(self)
        op_file = os.path.join(dir_data(), "ok_format_operator.txt")
        self.ns = optimized_solution.OptimizedSolution(op_file)
        self.ns.load_data()
    ...
    def test_longest_prefix(self):
        """ Test that the longest prefix is calculated correctly """
        self.assertEqual(self.ns.longest_prefix, 5)
